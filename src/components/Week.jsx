...
export class Week extends Component {
  ...
  fetchData = async () => {
    ...
    try {
      ...
      const fiveDayForecast = jsonData.filter((data) => {
        const forecastDate = new Date(data.dt_txt);
        const daysDifference = Math.floor(
          (forecastDate - currentDate) / (1000 * 3600 * 24)
        );
      ...
      localStorage.setItem("week", JSON.stringify(fiveDayForecast));
    } catch (error) {
